...
def search_for_file_path ():
    messagebox.showinfo('Response', 'Please select a CSV file to edit.')
    currdir = os.getcwd()
...
class DFEditor(QWidget):
    ...
    def __init__(self):
        ...
        button_mod.clicked.connect(self.areyousure)
    ...
    def export_to_csv(self):
        ...
        f = open(file_name)
        f.close()
        messagebox.showinfo('Response', 'CSV file has been overwriten!')
    
    def makemod(self):
        shutil.make_archive(output_filename, 'zip', dir_name)
        messagebox.showinfo('Response', 'Mod has been Made!')
        
    def areyousure(self):
        res = messagebox.askquestion("Form","Make sure you have Edited all of the CSV files, are you ready?")
        if res == 'yes':
            self.makemod()
        elif res == 'no':
            messagebox.showinfo('Response', 'Okay just making sure.')
        else:
